...
```{r}
trend_plots(forest_cluster, test)
```


```{r}
Y.forest = regression_forest(X, Y, clusters = train$areaid)
Y.hat = predict(Y.forest)$predictions
W.forest = regression_forest(X, W, clusters = train$areaid)
...
Y.hat = Y.hat, W.hat = W.hat,
clusters = train$areaid, samples_per_cluster = 10, tune.parameters = TRUE)
...
```{r}
biz_index_mat <- model.matrix(~ areaid + 0, data = train)
biz_index_size <- colSums(biz_index_mat)

dr.score = tau.hat + W / cf$W.hat * (Y - cf$Y.hat - (1 - cf$W.hat) * tau.hat) - (1 - W) / (1 - cf$W.hat) * (Y - cf$Y.hat + cf$W.hat * tau.hat)

biz_index_score = t(biz_index_mat) %*% dr.score / biz_index_size
total_exp = t(biz_index_mat) %*% X$total_exp_mo_pc_1 / biz_index_size
high_total_exp = total_exp > median(total_exp) 
t.test(biz_index_score[high_total_exp], biz_index_score[!high_total_exp]) 
