...
tm_gates <- function(target, treatment, data, 
  ...
  for (i in 1:num.iter) {
    ...
    auxi_index <- createDataPartition(data[,target], p = split_ratio, list = FALSE)
...
endline1_biz_wo <- endline1_biz %>%
  select(everything(), -starts_with("biz"), -contains("index"), biz_index_all_1)
```

#### With Random Forest
```{r}
tm_gates_biz_wo <- tm_gates("biz_index_all_1", "treatment", endline1_biz_wo,
...
#### With Causal Random Forest

```{r}
tm_gates_biz_wo_crf <- tm_gates("biz_index_all_1", "treatment", endline1_biz_wo,
                                   cluster="areaid", num.iter=1, ml_method="crf")
```

###### Results: Two-Models Approach
```{r}
summary(tm_gates_biz_wo_crf[[1]])
```

##### Results: GATES
```{r}
summary(tm_gates_biz_wo_crf[[2]])
```
