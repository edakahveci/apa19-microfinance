...
```{r}
rf_gates <- function(target, treatment, data, cluster=NULL, num.iter=100) {
  results <- list()
  ...
  for (i in 1:num.iter) {
    ...
    gates_formula <- as.formula(paste(target, "~",
                                      paste(data_col[!data_col %in% exclude_col], collapse = " + "),
                                      "|0|0|",
                                      cluster))
    model <- felm(gates_formula,
                  data = main,
                  weights = main$weight)
...
```{r}
gates_biz_clean <- endline1_biz %>%
  select(everything(), -contains("biz"), -contains("index"), biz_index_all_1)
str(gates_biz_clean)
```
```{r}
gates_biz_ <- rf_gates("biz_index_all_1", "treatment", 
                        gates_endline1_biz_without, cluster = "areaid", num.iter = 1)
summary(gates_biz_3[[1]])
```

```{r}
gates_biz_1_fix <- endline1_biz %>%
  select(spouse_literate_1, spouse_works_wage_1, hhsize_adj_1,
         women1845_1, anychild1318_1, old_biz, ownland_hyderabad_1, ownland_village_1,
         treatment, biz_index_all_1, areaid)
gates_biz_2 <- rf_gates("biz_index_all_1", "treatment", 
                        gates_biz_1_fix, cluster = "areaid", num.iter = 1)
summary(gates_biz_2[[1]])
```


```{r}
gates_biz_1 <- rf_gates("biz_index_all_1", "treatment", 
                        endline1_biz, cluster = "areaid", num.iter = 1)
summary(gates_biz_1[[1]])
```
...
X <- endline1_biz[ , -which(names(endline1_biz) %in% 
...
## Model with Clustering with Other Indexes

```{r}
y <- endline1_biz$biz_index_all_1
X <- endline1_biz[ , -which(names(endline1_biz) %in% 
                                 c("biz_index_all_1", "treatment","areaid"))]
W <- endline1_biz$treatment
...
y <- endline1_biz$biz_index_all_1
X <- endline1_biz[ , -which(names(endline1_biz) %in% 
                                 c("biz_index_all_1", "treatment", "hhid",
                                   "consumption_index_1", "social_index_1", "labor_index_1",
                                   "income_index_1", "credit_index_1", "women_emp_index_1",
...
## Variable Importance without Indexes

```{r}
var_imp_plot(forest_without)
```

```{r}
split_frequencies(forest_without, max.depth = 4)
```
...
colnames(X[61])
```
## Model without Indexes with Clustering

```{r}
y <- endline1_biz$biz_index_all_1
X <- endline1_biz[ , -which(names(endline1_biz) %in% 
                                 c("biz_index_all_1", "treatment","areaid", "hhid",
                                   "consumption_index_1", "social_index_1", "labor_index_1",
                                   "income_index_1", "credit_index_1", "women_emp_index_1",
                                   "home_durable_index_1"))]
W <- endline1_biz$treatment
...
forest_without_cluster <- causal_forest(
  ...
  mtry = 1, num.trees = 1000,
  clusters = matrix_1$areaid)
forest_without_cluster
```

## Variable Importance without Indexes with Clustering

```{r}
var_imp_plot(forest_without_cluster)
```
```{r}
split_frequencies(forest_without_cluster, max.depth = 4)
```
