...
endline1_fixed <- endline1[,1:15]
endline1_index <- endline1 %>%
  select(contains("index"))

endline1_biz <- cbind(endline1_fixed, endline1_index)

endline1_biz$hhid <- NULL
...
- Treatment Variable:  `treatment`
- Target Variable: `credit_index_1`

We want to find out whether there are heterogeneous effects of  "availibility of Spandana microcredit loan" on credit of households in the area.

```{r}
endline1 %>%
  group_by(treatment) %>%
  summarize("Num. of Obs." = n(),
            "Ave. Biz. Index" = mean(credit_index_1, na.rm = TRUE))
```
...
## Two-Models Approach & Sorted Groups Average Treatment Effect

### Results (using Random Forest)

```{r}
tm_gates_credit <- tm_gates("credit_index_1", "treatment", endline1_credit,
                            split_ratio = 0.6,
                            cluster="areaid", num.iter=1, ml_method="rf")
```

```{r}
summary(tm_gates_credit[[1]])
```

```{r}
summary(tm_gates_credit[[2]])
```
...
- Treatment Variable:  `treatment`
- Target Variable: `consumption_index_1`

We want to find out whether there are heterogeneous effects of  "availibility of Spandana microcredit loan" on consumption of households in the area.

```{r}
endline1 %>%
  group_by(treatment) %>%
  summarize("Num. of Obs." = n(),
            "Ave. Biz. Index" = mean(consumption_index_1, na.rm = TRUE))
```
...
### Results: Random Forset
```{r}
tm_gates_cons <- tm_gates("consumption_index_1", "treatment", endline1_consumption,
                          split_ratio = 0.6,
                          cluster="areaid", num.iter=1, ml_method="rf")
```

```{r}
summary(tm_gates_cons[[1]])
```

```{r}
summary(tm_gates_cons[[2]])
```
